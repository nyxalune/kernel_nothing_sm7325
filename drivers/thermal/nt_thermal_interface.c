...
static int of_parse_thermal_message(void)
{
	...
	np = of_find_node_by_name(NULL, "nt-thermal-interface");
	if (!np)
		return -EINVAL;
...
static int __init nt_thermal_interface_init(void)
{
	...
	node = of_find_node_by_name(NULL, "nt-thermal-interface");
	if (!node) {
		pr_err("%s: Cannot find nt-thermal-interface node\n", __func__);
		return ret;
	}
...
MODULE_AUTHOR("Nothing thermal team");
